...
module HasN
  ...
  # Returns the current association cache
  def association_cache
    @association_cache ||= {}
  end

  # Returns the specified association instance if it responds to :loaded?, nil otherwise.
  # @param [QueryProxy] query_proxy the QueryProxy to cache or lookup
  # @param [Enumerable] association_obj the HasN::Association object used to perform this query
  def association_instance_get(query_proxy, association_obj)
    return if association_cache.nil? || association_cache.empty?
    lookup_obj = query_proxy_string(query_proxy)
  ...
  def association_instance_set(query_proxy, collection_result, association_obj)
    cache_key = query_proxy_string(query_proxy)
    reflection = association_reflection(association_obj)
    @association_cache[reflection.name] = { cache_key => collection_result } unless reflection.nil?
  end
  ...
  # @param [Neo4j::ActiveNode::Query::QueryProxy] query_proxy
  # @return [String] A basic hash of the query
  def query_proxy_string(query_proxy)
    query_proxy.to_cypher_with_params.hash.abs
  end
