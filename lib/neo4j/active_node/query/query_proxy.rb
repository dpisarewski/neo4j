  ...
  module ActiveNode
    module Query
      class QueryProxy
        ...
        def enumerable_query(pluck_this)
          return self.pluck(pluck_this) if @association.nil? || caller.nil?
          association_collection = caller.association_instance_get(self, @association)
          if association_collection.nil?
            association_collection = self.pluck(pluck_this)
            caller.association_instance_set(self, association_collection, @association) unless association_collection.empty?
          end
        ...
        def each(node = true, rel = nil, &block)
          if node && rel
            self.pluck(identity, @rel_var).each { |obj, rel| yield obj, rel }
          else
        ...
        def to_cypher_with_params
          final_query = query.return_query([:result])
